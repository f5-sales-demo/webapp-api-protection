#!/usr/bin/env bash
# LB challenge_type (CH-1) live coverage-matrix harness.
#
# Cycles the LIVE webapp-api-protection LB through the challenge_pairs variant set and verifies
...
# append to reports/challenge-matrix.txt.
#
...
umask 077
trap 'rm -rf /tmp/ch-variants 2>/dev/null; rm -f /tmp/ch-*.log 2>/dev/null' EXIT
...
VARDIR=/tmp/ch-variants
REPORT=../reports/challenge-matrix.txt
mkdir -p ../reports
...
count=$(python3 ../scripts/challenge_pairs.py --emit "$VARDIR")
...
round_trip() {
  ...
  terraform import "${COMMON[@]}" -var-file="$vf" "$LB_ADDR" "$LB_ID" >/tmp/ch-import.log 2>&1 || {
    echo "FAIL $label lb-import ($(grep -iE 'error' /tmp/ch-import.log | head -1 | cut -c1-70))" >>"$REPORT"
    return
  }
  terraform plan -detailed-exitcode "${COMMON[@]}" -var-file="$vf" >/tmp/ch-rt-plan.log 2>&1
  case $? in
...
while read -r idx name vf _flag; do
  ...
  echo "--- $label ---"
  varfile="${VARDIR}/${vf}"
  if ! terraform apply -auto-approve "${COMMON[@]}" -var-file="$varfile" >/tmp/ch-apply.log 2>&1; then
    echo "FAIL $label apply ($(grep -iE 'error' /tmp/ch-apply.log | head -1 | cut -c1-80))" >>"$REPORT"
    continue
  fi
  terraform plan -detailed-exitcode "${COMMON[@]}" -var-file="$varfile" >/tmp/ch-plan.log 2>&1
  case $? in
...
echo "===== CHALLENGE MATRIX RESULTS ($START..$END) ====="
cat "$REPORT"
